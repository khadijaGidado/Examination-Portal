...
$exidenti = $_GET['examid'];
 $sql="SELECT invitestudents.Examid, fillquestions.Examid, fillquestions.question_number,fillquestions.que FROM invitestudents 
 INNER JOIN fillquestions ON invitestudents.Examid=fillquestions.Examid INNER JOIN users
...
<?php 
        if (isset($_POST['submit'])){
          ...
          $getting="SELECT Examid, question_number, que FROM fillquestions WHERE Examid=$exidenti";
          ...
          while($row=mysqli_fetch_assoc($queryvariable)){
           ...
           $submit="INSERT INTO fillresponse (`Examid`, `questio_no`, `answer`, `userid`) 
...
<div class="card text-white bg-light mb-12" style="max-width: 50rem; color:black!important;">
  <div class="card-header"><?php  echo  $row['que']; ?></div>
  <div class="card-body">
  <div class="form-group shadow-textarea">
  <textarea class="form-control z-depth-1" id="exampleFormControlTextarea6" rows="3" name="<?php echo  $row['question_number']; ?>" placeholder="Students should type answer here..."></textarea>
</div>
  </div>
</div>
<?php
